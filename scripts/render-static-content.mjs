...
const readDistHtml = (fileName) => fs.readFile(path.join(distRoot, fileName), "utf8");
const writeDistHtml = (fileName, html) => fs.writeFile(path.join(distRoot, fileName), html);
...
const renderIndex = async () => {
  ...
  let html = await readDistHtml(fileName);

  const bioMarkdown = await readUtf8("public/contents/bio.md");
  ...
  for (const injection of injections) {
    html = injectStaticContent({ html, fileName, ...injection });
  }

  assertNoLoadingText({ html, fileName });
  injections.forEach(({ id }) => requireStaticMarker({ html, fileName, id }));
  await writeDistHtml(fileName, html);
};

const renderPublications = async () => {
  const fileName = "publications.html";
  let html = await readDistHtml(fileName);
  ...
  html = injectStaticContent({ html, fileName, ...injection });
  assertNoLoadingText({ html, fileName });
  requireStaticMarker({ html, fileName, id: injection.id });
  await writeDistHtml(fileName, html);
};

try {
  await fs.access(path.join(distRoot, "index.html"));
  await fs.access(path.join(distRoot, "publications.html"));
  await renderIndex();
  await renderPublications();
  console.log("Static content rendered into dist HTML.");
} catch (error) {
