...
const renderPublicationsSection = async () => {
  ...
  try {
    const items = await loadList({ url: "contents/publications.md" });
    const withIds = assignPublicationIdsByType(items);
    const selected = withIds.filter(
      (item) => String(item?.isSelected || "").trim().toLowerCase() === "true"
    );
    ...
    renderItems({
      ...
      items: selected.sort(compareByDateDesc),
